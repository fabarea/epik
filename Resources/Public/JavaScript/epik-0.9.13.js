...
function EPIK(formId, options) {
    ...
    this.CONSTANTS.FORM_FIELD.SID = "sid";
    this.CONSTANTS.FORM_FIELD.KEYWORD = "keyword";
    ...
    configOfFields.popup = {hidden: true, mandatory: false, sendToEpp: true};
    ...
    configOfFields.sid = {hidden: true, mandatory: false, sendToEpp: true};
    configOfFields.keyword = {hidden: false, mandatory: false, sendToEpp: true};
    ...
    */
    this.updateCookie = function(name, value, prefix) {
        ...
        var paramName = name.encode();
        var paramValue = null;
        if(name !== 'cardno' && name !== 'cvv' && name !== 'expy' && name !== 'expm') {
   ...
   /**
    * Sets Shortnumber sid
    *
    * @param sid
    */
    this.setSID = function(sid){
        that.options.sid = sid;
        that.updateCookie(that.CONSTANTS.FORM_FIELD.SID, sid, that.formKey);
        updateValueOfField(that.CONSTANTS.FORM_FIELD.SID, sid);
    };
    ...
    */
    this.setKeyword = function(keyword){
        ...
        that.updateCookie(that.CONSTANTS.FORM_FIELD.KEYWORD, keyword, that.formKey);
        updateValueOfField(that.CONSTANTS.FORM_FIELD.KEYWORD, keyword);
    };
   ...
   /**
    * Gets short number sid
    *
    * @returns sid
    */
    this.getSID = function(){
        return that.options.sid;
    };
    ...
    */
    ...
    this.waitUntilLoaded = function(iFrameO, iFrameFormO, waitingPageO) {
        ...
        if(that.options.iframe_loaded) {
            iFrame.contents().find("body").append(iFrameForm);
            that.options.iframe_loaded = false;
            iFrame.contents().find("body form").submit();
        } else if(waitingPage == "#") {
            setTimeout(function() {
            iFrame.contents().find("body").append(iFrameForm);
    ...
    */
    this.waitForConfirmation = function (options){
        that.stopCounter = false;
        that.stopPolling = false;
    ...
    this.abortWaitForConfirmation = function () {
        if (that.countTimer !== null) {
            clearTimeout(that.countTimer);
        }
        if (that.pollTimer !== null) {
    ...
    */
    this.parseStatusResponse = function(data, merchantTransactionId, transactionId, confirmedFunction, declinedFunction, interval) {
        ...
        if (data["status"] == "pending") {
            if (!that.stopPolling) {
                that.pollTimer = setTimeout(function () {that.pollStatus(merchantTransactionId, transactionId, confirmedFunction, declinedFunction, interval);}, interval * 1000);
            }
...
EPIK.utils = {
    /**
     * Gets the InternetExplorer version (returns -1 if non-IE)
     * @returns {float} two-part IE version like 5.5 or 9; -1 for non-IE
     */
    ...
    /**
     * Normalizes the given MSISDN with the following rules:
     * - removes all spaces
     * - must be a swiss (+41 or 0041) number
     * - must be a mobile number (079,078,077,076)
     * - must contain 7 non-prefix digits
     * @param {string} msisdn - the MSISDN that should be normalized
     * @returns {string} The normalized MSISDN; null if the MSISDN is not valid
     */
    normalizeMSISDN: function(msisdn) {
        msisdn = msisdn.replace(/\s/g, "");

        var match = /^(\+41|0041|0)(7[6789][0-9]{7})$/g.exec(msisdn);
    ...
    /**
     * Gets the credit card type from the prefix of the given (partial) credit
     * card number.
     * @param {type} cardno - credit card number (can be partial, only first 2
     * digits are required)
     * @returns {String} 'vis' for VISA, 'eca' for MasterCard, 'amx'
     * for AmericanExpress and null for any non-recognized number.
     */
    getCreditCardType: function(cardno) {
        if (cardno.match(/^(4)/g)) {
            return 'vis';
        } else if (cardno.match(/^(5[0-5])/g)) {
            return 'eca';
        } else if (cardno.match(/^3[4|7]/g)) {
            return 'amx';
        }
        return null;
    }
